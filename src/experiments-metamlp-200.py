# Only modification compared to experiments-200: uses pretrained meta-GamutMLP model/weights and changing iter from 9000 to 1200
# reproduce Table 1 for meta MLP (23KB)+enc
...
if __name__ == "__main__":
    ...
    for img_path in image_paths:
        ...
        checkpoint = torch.load('src/meta-mlp_state_dict-100.pt')       # <<< MODIFIED: load from a checkpoint
        ...
        for iter in range(1200):                                        # <<< MODIFIED: num of optimization iterations
        ...
        curr_path = os.getcwd()
        folder_name = 'restored/meta-mlp23'
    ...
    with open(f'{folder_name}/meta-mlp-23KB-experiment.txt', 'w') as f:
...
#         # build network
#         # TODO: LOAD PRETRAINED MODEL FROM CHECKPOINT
#         net = GamutMLP().to(device)
#         checkpoint = torch.load('src/meta-mlp_state_dict-100.pt')
#         net.load_state_dict(checkpoint)
#         loss_fn =  torch.nn.MSELoss()
#         learning_rate = 0.001
#         optimizer = torch.optim.Adam(net.parameters(), lr=learning_rate)
...
#         print('training start')        
#         start_time = time.time() # optimization start time
#         for iter in range(1200): # num of optimization iterations
#         # for iter in range(10): # num of optimization iterations
#             net.train() # good practice, some networks have training dependent behaviour
#             optimizer.zero_grad()
#             x = torch.tensor(training_input).float().to(device)
#             pred_residuals = net(x)
...
#         file_name_w_ext = os.path.basename(img_path)
#         print('saved image as: ', file_name_w_ext)
#         cv2.imwrite(f'RESTORED_{file_name_w_ext}.png', restored_I_PP)
...
#     # print out the average metrics
#     # TODO: rename file to 'experiments-200-23KB.py'
#     # - run this file on 20 images
#     # - run 'experiments-200-53KB' on 20 images
#     # ^^^ finished top two rows
#     print("="*80)
#     print('DONE')
#     print('>>> AVERAGE RMSE: ', sum(rmse_loss)/len(rmse_loss))
#     print('>>> AVERAGE RMSE OG: ', sum(rmse_og_loss)/len(rmse_og_loss))
